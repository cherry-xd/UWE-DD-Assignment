...
void draw_pixel(uint8_t x, uint8_t y, char colour) {
    display_draw_pixel(x, y, fill=colour); 
}

void draw_line_not_all_slopes(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, char colour='black') {
    uint8_t dx = x1 - x0;
    uint8_t dy = y1 - y0;
    uint8_t D = 2*dy - dx;
}

void draw_line_low(uint8_t x0, int8_t y0, uint8_t x1, uint8_t y1, char colour) {
    uint8_t dx = x1 - x0;
    uint8_t dy = y1 - y0;
    uint8_t yi = 1;
    if(dy < 0) {
...
void draw_line_high(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, char colour) {
    uint8_t dx = x1 - x0;
    uint8_t dy = y1 - y0;
    uint8_t xi = 1;
    if(dx < 0) {
...
void draw_line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, char colour) {
    if(abs(y1 - y0) < abs(x1 - x0)) {
        if(x0 > x1) {
            draw_line_low(x1, y1, x0, y0, colour);
        }
        else {
            draw_line_low(x0, y0, x1, y1, colour);
        }
    }
    else {
        if(y0 > y1) {
            draw_line_high(x1, y1, x0, y0, colour);
        }
        else {
            draw_line_high(x0, y0, x1, y1, colour);
        }
    }
}
